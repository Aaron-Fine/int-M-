...
| PR 2 — Allocation-free scalar kernel                           | B                 | **Landed**; speed gate pending Stage A                       | pr2 microbench (directional)                               |
| PR 3 — Common verifier and semantic oracle                     | C (part)          | **Landed**                                                   | src/domain/verifier.ts, verifier/adjudication tests        |
| PR 4 — Checkpoint candidates                                   | C                 | **Landed** behind the `legacy-scan` default; Stage A pending | src/domain/checkpoint.ts, pr4 bench (directional)          |
| PR 5 — Period policy buckets and product language              | D                 | **Landed**                                                   | src/domain/period-policy.ts, policy tests                  |
| Stage A — target-hardware baseline and gate runs               | A/B/C/D           | **In progress** (first run being produced; not complete)     | evidence/phase-2/ contract; no artifacts yet               |
| Conditional PRs M, O, N, E, G, F, H, I, K (+ renderer details) | M/O/N/E/G/F/H/I/K | Not started as PRs; PoC evidence state varies (below)        | poc/performance/results/, poc/performance/browser/results/ |
...
Stage A is the first benchmark-contract execution of plan §9 on the declared
target laptop class: production bundle, stable branded Chrome and Firefox,
frozen corpus v1, repetition counts and BCa paired intervals per the protocol,
artifacts under `evidence/phase-2/<date>-<commit>/` per the
[evidence contract](../../evidence/phase-2/README.md). **It is in progress;
another work stream is producing the first run. No Stage A artifact exists in
this repository yet, and no gate that depends on it is decided.**
...
| --- | ---------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| M   | Conjugate symmetry mirroring                         | Not started as a PR. Browser PoC directional ([conjugate-mirror.json](../../poc/performance/browser/results/conjugate-mirror.json)): full-over-half+mirror wall ratio 1.574 (easy default) and 1.653 (symmetric hard view), mirror fill ~4 ms median — near the ≥1.6× bar but one browser, one machine, headless. The harness parity variant is specified but not implemented (PoC README roadmap).                                                                                                                                                            |
| O   | Cross-profile semantic carryover                     | Not started; no PoC measurement.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| N   | Cost-weighted static banding                         | Not started as a PR. Browser PoC directional ([coarse-cost.json](../../poc/performance/browser/results/coarse-cost.json)): coarse-pass cost estimates recorded with the equal-cost boundaries N would ship; recorded decision ties viability to the skew gate firing and estimate correlation.                                                                                                                                                                                                                                                                 |
...
| F   | Reproducible period-12 core and runtime shards       | **Deferred by default** per plan rev 3/4 (workstream F disposition) and [ADR 0004](decisions/0004-catalog-disposition.md). Reopened only if C and the Newton PoC quantify seed value checkpoint detection alone cannot provide.                                                                                                                                                                                                                                                                                                                                |
