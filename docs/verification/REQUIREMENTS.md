...
`poc/performance` pr2 microbench). Requirements not touched by PR 1 or
PR 2 have no assigned PR yet.
...
| `MI-PERF-002` | `MI-VER-PERF-002`      | `src/domain/orbit.ts`, `src/render/classify-rows.ts`, period policy                | PR 2 (classifier core); period policy unassigned       | Scalar differential suite (`tests/unit/domain/orbit-scalar-parity.test.ts`), PoC control parity (`poc/performance/src/control.test.ts`), pr2 microbench (`poc/performance/results/pr2/`); oracle/differential fixtures, verifier tests, policy UI tests for the period-policy part | Open           |
...
| `MI-PERF-004` | `MI-VER-PERF-002`      | Classifier acceptance paths (`src/render/`, `src/domain/`)                         | PR 2 (acceptance paths unchanged; parity re-evidenced) | PoC control parity and pr2 full-raster parity gate (`poc/performance/src/pr2-bench.ts`), oracle fixtures, candidate rejection tests, continuous-field tolerance report                                                                                                             | Open           |
