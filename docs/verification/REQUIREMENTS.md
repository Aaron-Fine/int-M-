...
| `MI-PERF-002` | `MI-VER-PERF-002`      | `src/domain/orbit.ts`, `src/domain/verifier.ts`, `src/render/classify-rows.ts`, period policy | PR 2 (classifier core); PR 3 (verifier-gated acceptance policy `src-verifier-1.0.0`); period policy unassigned | Scalar differential suite (`tests/unit/domain/orbit-scalar-parity.test.ts`), common-verifier unit tests (`tests/unit/domain/verifier.test.ts`), PoC control parity (`poc/performance/src/control.test.ts`), pr2 microbench (`poc/performance/results/pr2/`); oracle/differential fixtures, policy UI tests for the period-policy part                                            | Open           |
...
| `MI-PERF-004` | `MI-VER-PERF-002`      | Classifier acceptance paths (`src/domain/verifier.ts`, `src/domain/orbit.ts`)                 | PR 3 (acceptance migrated to the common verifier; analytic fast paths included)                                | Common-verifier unit tests (`tests/unit/domain/verifier.test.ts`), dd-oracle adjudication of every legacy-vs-verifier divergence (`tests/unit/domain/orbit-oracle-adjudication.test.ts`), PoC control parity and pr2 full-raster parity gate (`poc/performance/src/pr2-bench.ts`), oracle fixtures, candidate rejection tests (PR 4+ sources), continuous-field tolerance report | Open           |
